...
Doorkeeper.configure do
  ...
  resource_owner_authenticator do
    ...
    if client_is_valid?(params[:client_id], params[:client_secret])
      user_by_email(email)
    end
  end
  ...
  #   email = decrypt(request.params[:username])
  #   login_token = decrypt(request.params[:password])
  #   client_id = params[:client_id]
  #   user = get_user_by_email(email)
  #   if client_is_valid?(client_id) && user_is_authorized?(user, login_token)
  #     record_session(user)
  #   end
  # end
  ...
  authorization_code_expires_in 15.minutes
